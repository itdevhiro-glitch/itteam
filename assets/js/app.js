const ADMIN_UID = "Ogy9lUbGHbSu8wYIYx2gQsTtFDF2";
    ...
    window.resetUserPassword = async () => {
        ...
        const e = $('#ud_email').value;
        if (!uid || !e) return alert('Pilih user terlebih dahulu.');
        if (!confirm(`Reset password ${e} ke default ${DEFAULT_EMPLOYEE_PASSWORD}?`)) return;
        showLoading();
        try {
            await db.ref(`password_reset_requests/${uid}`).set({
                uid,
                email: e,
                defaultPassword: DEFAULT_EMPLOYEE_PASSWORD,
                requestedBy: auth.currentUser ? auth.currentUser.uid : 'admin',
                status: 'pending-admin-sdk',
            ...
            showToast('Request reset dibuat. Jalankan Cloud Function/Admin SDK agar password Auth benar-benar berubah.');
        } catch (er) {
            alert(er.message);
        } finally {
            hideLoading();
